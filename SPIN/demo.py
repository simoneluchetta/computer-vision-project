...
parser.add_argument('--bbox_dir', type=str, default=None,
                    help='Path to .json file directory containing bounding box coordinates')
parser.add_argument('--openpose_dir', type=str, default=None,
                    help='Path to .json directory containing openpose detections')
...
parser.add_argument('--result_folder', default=None,
                    help='If set, save detections to a .npz file inside a folder')
...
def bbox_from_openpose(openpose_file, rescale=1.2, detection_thresh=0.2):
    ...
    bbox_size = (valid_keypoints.max(axis=0) -
                 valid_keypoints.min(axis=0)).max()
...
if __name__ == '__main__':
    ...
    renderer = Renderer(focal_length=constants.FOCAL_LENGTH,
                        img_res=constants.IMG_RES, faces=smpl.faces)

    # sorted(os.listdir(whatever_directory))
    img_dir = sorted_alphanumeric(os.listdir(args.img_dir))
    json_dir = sorted_alphanumeric(os.listdir(args.openpose_dir))
    bbox = None

    # Store SMPL parameters
    smpl_pose = np.zeros((1, 72))
    smpl_betas = np.zeros((1, 10))
    smpl_camera = np.zeros((1, 3))
    pred_joints = np.zeros((1, 17, 3))

    # Check if folders are the same size
    if len(img_dir) != len(json_dir):
        raise TypeError("Folders should be the same size!")

    for i in range(0, len(img_dir)):
        # Select images and json with same id
        img = os.path.join(args.img_dir, img_dir[i])
        openpose = os.path.join(args.openpose_dir, json_dir[i])

        # Preprocess input image and generate predictions
        img, norm_img = process_image(
            img, bbox, openpose, input_res=constants.IMG_RES)
        with torch.no_grad():
        ...
        rot_pad = torch.tensor(
            [0, 0, 1], dtype=torch.float32, device=device).view(1, 3, 1)
        rotmat = torch.cat((pred_rotmat.view(-1, 3, 3),
                            rot_pad.expand(24, -1, -1)), dim=-1)
        pred_pose = tgm.rotation_matrix_to_angle_axis(
            rotmat).contiguous()
        pred_pose = pred_pose.view(-1, 72)

        # Select only 24 joints we need for Anerf
        smpl_joints = pred_output.joints[:, constants.J24_FLIP_PERM, :].cpu().numpy()[
            0]

        smpl_pose = pred_pose.cpu().numpy()
        smpl_betas = pred_betas.cpu().numpy()
        smpl_camera = pred_camera.cpu().numpy()

        # Calculate camera parameters for rendering
        camera_translation = torch.stack([pred_camera[:, 1], pred_camera[:, 2], 2 *
                                          constants.FOCAL_LENGTH/(constants.IMG_RES * pred_camera[:, 0] + 1e-9)], dim=-1)
        ...
        img_shape_side = renderer(
            rot_vertices, camera_translation, np.ones_like(img))

        outfile = img_dir[i].split(
            '.')[0] if args.outfile is None else args.outfile

        if not os.path.isdir(args.result_folder):
            os.mkdir(args.result_folder)

        # Save reconstructions
        cv2.imwrite(args.result_folder + outfile + '_shape.png', 255 * img_shape[:, :, ::-1])
        cv2.imwrite(args.result_folder + outfile + '_shape_side.png',
                    255 * img_shape_side[:, :, ::-1])
